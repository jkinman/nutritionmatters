...
;(function($) {
    ...
    RelatedPosts.prototype.displayRelated = function(posts) {
        ...
        related.forEach(function(post) {
            if (count < self.options.limit) {
                $(self.element).append($('<li><img src="' + post.media + '"><h5><a href="' + post.url + '">' + post.title + '</a></h5><time>' + post.date.substring(0, post.date.length - 12) + '</time></li>'));
            }
            count++;
        });
    ...
    RelatedPosts.prototype.getPosts = function(feeds) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (item.find('title').text() !== this.getCurrentPostTitle(this.options.titleClass) && item.find('media\\:content, content').attr('url')) {

                posts.push({
                    title: item.find('title').text(),
                    url: item.find('link').text(),
                    media: item.find('media\\:content, content').attr('url'),
